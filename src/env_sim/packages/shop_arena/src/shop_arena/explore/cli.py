"""Command-line entrypoint for ``shop_arena.explore``.

Implements the §5.11 CLI surface of
``docs/specs/shop_arena/shop_arena.explore.md``. v0.1 wires two debug paths:
...
:mod:`shop_arena.explore.pipeline`. Both the default and the
``--synthesize-only`` paths route the §5.10 manual-merge LLM call
through the configured harness runtime via
:func:`shop_arena.explore.pipeline.build_runtime_llm` — when ``--runtime``
...
from shop_arena.explore.config import (
    DEFAULT_MAX_ITERS,
...
from shop_arena.explore.pipeline import build_runtime_llm, explore
...
from shop_arena.explore.synthesize import (
    SynthesisError,
)
from shop_arena.explore.synthesize import (
...
def _run_prefetch_only(args: argparse.Namespace) -> int:
    ...
    capture_navigation(args.url, dest_dir=dest_dir)
    capture_hero(args.url, dest_dir=dest_dir)
    print(
...
def _run_explore(args: argparse.Namespace) -> int:
    ...
    * ``--force-resume`` is forwarded to the harness via
      :class:`shop_arena.explore.config.ExploreConfig` so the §5.5 refusal
