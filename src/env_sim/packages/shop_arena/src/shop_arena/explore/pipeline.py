"""Pipeline orchestrator for ``shop_arena.explore`` runs.

Implements §5.2 + §5.10 of the ShopExplore spec
(``docs/specs/shop_arena/shop_arena.explore.md``): the public :func:`explore`
function takes a validated :class:`~shop_arena.explore.config.ExploreConfig`,
...
:func:`shop_arena.explore.synthesize.synthesize` to publish the four
...
from shop_arena.explore.config import ExploreConfig, ExploreResult
from shop_arena.explore.prefetch import capture_hero, capture_navigation
from shop_arena.explore.prefetch import run as run_prefetch
from shop_arena.explore.synthesize import LLMClient, SynthesisError, synthesize
...
def explore(config: ExploreConfig, *, llm: LLMClient | None = None) -> ExploreResult:
    ...
    2. Run :func:`shop_arena.explore.prefetch.run` into a temporary seed dir
    ...
    6. Run :func:`shop_arena.explore.synthesize.synthesize` to publish
       ``manual.md`` / ``capabilities.json`` / ``stats.json`` /
    ...
    try:
        ...
        capture_navigation(config.url, dest_dir=seed_dir / "prefetch")
        capture_hero(config.url, dest_dir=seed_dir / "prefetch")
        ...
        if config.runtime == "pi" and skill_dir is not None:
            runtime_kwargs["skill_paths"] = [skill_dir]
...
def build_runtime_llm(runtime: AgentRuntime, *, timeout: float) -> LLMClient:
    ...
    Returns:
        ...
        :func:`shop_arena.explore.synthesize.synthesize`.
...
def _render_agents_md(template: str) -> str:
    ...
    ``shop_arena.explore.capabilities.schema`` so it can never drift from the
    pydantic model.
...
def _derive_playwright_session(run_dir: Path) -> str:
    ...
    iteration, and parallel ``shop_arena.explore`` runs against the same
