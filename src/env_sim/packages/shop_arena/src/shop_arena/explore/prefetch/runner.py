...
_FETCH_PLAN: tuple[tuple[str, str], ...] = (
    ...
    ("/collections.json?limit=250", "collections.json"),
...
_COLLECTIONS_INDEX = 3
"""Index in :data:`_FETCH_PLAN` of the ``/collections.json`` entry, after
which per-collection membership is captured into ``products_collections.json``
by :func:`_fetch_products_by_collection`."""
    ...
    *,
    ...
    try:
        for index, (path, filename) in enumerate(_FETCH_PLAN):
            ...
            # After /collections.json, capture per-collection membership into
            # products_collections.json so the ingest step can use ground-truth
            # membership instead of the token-matching heuristic.
            if index == _COLLECTIONS_INDEX:
                entries.extend(
                    _fetch_products_by_collection(
                        http_client,
                        base_root=base_root,
                        dest_dir=dest_dir,
                        pacer=pacer,
...
def _fetch_products_by_collection(
    ...
    *,
    ...
    """Capture ground-truth product→collection membership to disk.

    Reads the ``collections.json`` already written to ``dest_dir`` by the
    ``/collections.json`` fetch, then walks every page of each collection's
    ``/collections/{handle}/products.json`` feed and records which product
    handles belong to it. The merged result is written once to
    ``dest_dir/products_collections.json`` as a flat object mapping each
    collection handle to its member product handles::

        {"summer": ["linen-shirt", "straw-hat"], "winter": ["wool-coat"]}

    Membership handles are all the ingest step needs (see
    :class:`shop_arena.gen.data_synth.ingest.IngestCatalogStep`), so full
    product records are intentionally not persisted here — they already live
    verbatim in ``products.json``.

    Per-collection pagination mirrors :func:`_fetch_products_paginated`: a
    page returning fewer than :data:`PRODUCTS_PAGE_LIMIT` products, a
    non-200 / malformed body, or a transport error stops that collection,
    and :data:`PRODUCTS_MAX_PAGES` caps the walk. Every failure is
    non-fatal — the collection is recorded with the handles gathered so far
    (possibly none) and the run continues.

    Args:
        client: Pre-configured ``httpx.Client``; not closed here.
        base_root: ``scheme://host`` root from :func:`_normalize_base`.
        dest_dir: Destination directory. Reads ``collections.json`` and
            writes ``products_collections.json`` here.
        pacer: Shared rate pacer used by the parent run.
        user_agent: ``User-Agent`` header for every page request.
        timeout: Per-request timeout in seconds.

    Returns:
        One :class:`PrefetchEntry` per page actually requested, across all
        collections. The first entry that contributes to the written file
        carries ``saved_to="products_collections.json"`` so the artifact has
        a discoverable origin in ``prefetch.json``. Empty when
        ``collections.json`` is missing or lists no handles.
    """
    handles = _read_collection_handles(dest_dir / "collections.json")
    if not handles:
        return []
    ...
    for handle in handles:
        ...
        for page in range(1, PRODUCTS_MAX_PAGES + 1):
            pacer.wait()
            path = f"/collections/{handle}/products.json?page={page}&limit={PRODUCTS_PAGE_LIMIT}"
            absolute = base_root + path
            try:
                response = client.get(
                    absolute,
            ...
            if not saved_marked:
                entries.append(
                    base_entry.model_copy(update={"saved_to": "products_collections.json"}),
                )
                saved_marked = True
            else:
                entries.append(base_entry)

            member_handles.extend(
                str(item["handle"]) for item in page_products if isinstance(item.get("handle"), str)
            )
            if len(page_products) < PRODUCTS_PAGE_LIMIT:
                break

        membership[handle] = member_handles

    target = dest_dir / "products_collections.json"
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_bytes(
...
def _read_collection_handles(path: Path) -> list[str]:
    """Return the ordered collection handles from a ``collections.json`` file.

    Returns an empty list when the file is absent or its body is not the
    expected ``{"collections": [{"handle": ...}, ...]}`` shape — every
    malformed case is treated as "no handles" so the caller degrades to a
    no-op, consistent with the rest of the post-gate prefetch.
    """
    if not path.exists():
        return []
    try:
    ...
    if not isinstance(raw, dict):
        return []
    items = cast(dict[str, Any], raw).get("collections")
    if not isinstance(items, list):
        return []
    handles: list[str] = []
    for item in cast(list[Any], items):
        if not isinstance(item, dict):
            continue
        handle = cast(dict[str, Any], item).get("handle")
