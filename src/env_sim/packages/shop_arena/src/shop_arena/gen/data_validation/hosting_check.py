...
listed in ``docs/specs/shop_arena/shop_arena.gen.md`` §5.4 over real HTTP,
...
``packages/shop_backend/`` (Node + graphql-yoga). ``shop_arena.gen`` owns
...
3. ``Query.collections(first: 50)`` — surface resolves; a count that
   differs from the on-disk target (page cap or filtering) logs a
...
8. Image surface: for a locally served (relative) src, ``GET
   /images/<sample>`` returns 200 + ``image/*``; for a verbatim absolute
   CDN src (``ingest`` catalogs), the product's ``featuredImage.url`` is
   asserted to pass through as an absolute URL instead of fetching the CDN.
...
* ``inputs``: a :class:`~shop_arena.gen.steps.base.StepInput` for the
  upstream ``validate_schema`` step + one
  :class:`~shop_arena.gen.steps.base.FileInput` per ``data/*.json`` file.
...
_ABSOLUTE_URL_RE: Final[re.Pattern[str]] = re.compile(r"^https?://", re.IGNORECASE)
"""Absolute http(s) src. Verbatim ``ingest`` catalogs preserve remote CDN image
URLs, which ``shop_backend`` serves by passing through unchanged (see
``rewriteImageUrl`` in ``builders.ts``) rather than via the local ``/images/``
route."""

_IMAGE_EXTENSIONS: Final[tuple[str, ...]] = (".png", ".jpg", ".jpeg", ".webp", ".svg")
"""Recognised image extensions for locally served (relative) image srcs."""
...
def _check_collections(client: httpx.Client, *, target: int) -> dict[str, Any]:
    ...
    # A single first page is capped at ``_PAGE_SIZE``, so catalogs with more
    # collections legitimately return fewer nodes here than on disk. Catalog
    # completeness is enforced upstream by ``validate_schema``; a count
    # mismatch on this surface is a warning, not a hosting failure.
    if len(nodes) != target:
        _LOGGER.warning(
            "collections(first: %d) returned %d; %d on disk (page cap or filtering)",
            _PAGE_SIZE,
            len(nodes),
            target,
        )
...
def _check_image(
    ...
    *,
    ...
    name = "image_get"
    sample = _pick_image(products)
    if sample is None:
        return _failure(name, "no product image present in dataset")
    handle, src = sample
    if _ABSOLUTE_URL_RE.match(src):
        # Verbatim ``ingest`` catalogs keep absolute CDN srcs; there is no local
        # ``/images/`` file to fetch, and hitting the CDN would make the check
        # network-dependent. Verify the GraphQL passthrough instead.
        return _check_remote_image(client, handle=handle)
    url_path = f"/images/{src}"
    try:
...
def _check_remote_image(client: httpx.Client, *, handle: str) -> dict[str, Any]:
    """Verify remote CDN image passthrough for a verbatim ``ingest`` catalog.

    Ingested catalogs keep absolute CDN image srcs verbatim, and
    ``shop_backend`` serves them by returning the URL unchanged (see
    ``rewriteImageUrl`` in ``builders.ts``) rather than via the local
    ``/images/`` route. This asserts the GraphQL layer exposes the featured
    image of ``handle`` as an absolute URL — the served surface for remote
    images — without fetching the CDN.
    """
    name = "image_get"
    query = f"""
    {{
    ...
    if not isinstance(url, str) or not _ABSOLUTE_URL_RE.match(url):
        return _failure(
            name,
            f"product(handle: {handle!r}) featuredImage.url is not an absolute URL: {url!r}",
        )
    return _success(name, f"remote CDN passthrough: handle={handle!r} url={url!r}")
...
def _pick_image(products: list[dict[str, Any]]) -> tuple[str, str] | None:
    """Return ``(handle, src)`` for the first usable product image.

    A src is usable when it is an absolute http(s) CDN URL (verbatim
    ``ingest`` catalogs preserve remote srcs) or a relative path whose
    extension — ignoring any ``?v=`` cache-buster query suffix — is a
    recognised image type (LLM-synthesized catalogs served under
    ``/images/``). The owning product ``handle`` is returned so the remote
    branch can verify GraphQL passthrough.
    """
    for product in products:
        handle = product.get("handle")
        images = cast("list[dict[str, Any]] | None", product.get("images"))
        if not isinstance(handle, str) or not isinstance(images, list):
            continue
        for image in images:
            src = image.get("src")
            ...
            if _ABSOLUTE_URL_RE.match(src):
                return handle, src
            if src.split("?", 1)[0].lower().endswith(_IMAGE_EXTENSIONS):
                return handle, src
    return None
