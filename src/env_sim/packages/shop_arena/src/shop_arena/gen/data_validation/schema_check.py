...
the synthesizer uses on output (:mod:`shop_arena.gen.data_synth.schema`).
...
(:class:`shop_arena.gen.data_validation.hosting_check.ValidateHostingStep`,
...
upstream :class:`shop_arena.gen.data_synth.assemble.AssembleDataStep`.
...
* ``inputs``: :class:`~shop_arena.gen.steps.base.StepInput` refs for the
  upstream ``assemble_data`` and ``ingest_homepage`` steps + one
  :class:`~shop_arena.gen.steps.base.FileInput` per ``data/*.json`` file.
* ``outputs``: ``[]`` — verdict is in-memory (raise on failure).
* ``depends_on``: ``[assemble_data, ingest_homepage]``.
...
_UPSTREAM_HOMEPAGE: Final[str] = "ingest_homepage"
...
_IN_HOMEPAGE: Final[Path] = _DATA_DIR / "homepage.json"
...
def validate_data_dir(data_dir: Path) -> None:
    ...
    Args:
        data_dir: Directory containing the published files
            (typically ``<out_dir>/data/``). ``homepage.json`` is
            validated only when present (optional file).
    ...
    Raises:
        ...
        SchemaValidationError: A file is not valid JSON, has the wrong
            ...
            :mod:`shop_arena.gen.data_synth.schema`.
    """
    ...
    # homepage.json is optional (empty when the hero capture did not run, and
    # absent altogether in pre-homepage datasets); validate only when present.
    if (data_dir / _IN_HOMEPAGE.name).exists():
        _validate_object(data_dir / _IN_HOMEPAGE.name, model=Homepage)
...
class ValidateSchemaStep:
    ...
    :mod:`shop_arena.gen.data_synth.schema`. The verdict is in-memory: a
    ...
    Attributes:
        ...
        phase: ``data_validation``.
        inputs: :class:`StepInput` refs for ``assemble_data`` and
            ``ingest_homepage`` plus a :class:`FileInput` per
            ``data/*.json`` file.
    ...
    def __init__(self) -> None:
        """Build the step bound to its upstream data producers.

        Depends on both ``assemble_data`` (store / products / collections /
        pages / policies / navigation) and ``ingest_homepage``
        (``homepage.json``) so every published file exists before validation.
        """
        self.id: str = _STEP_ID
        self.phase: str = _PHASE
        self.inputs: list[InputRef] = [
            StepInput(step_id=_UPSTREAM_ASSEMBLE),
            StepInput(step_id=_UPSTREAM_HOMEPAGE),
            *(FileInput(path=p) for p in _IN_FILES),
        ]
        self.outputs: list[Path] = []
        self.depends_on: list[str] = [_UPSTREAM_ASSEMBLE, _UPSTREAM_HOMEPAGE]
...
def _validate_object(path: Path, *, model: type[Store] | type[Navigation] | type[Homepage]) -> None:
