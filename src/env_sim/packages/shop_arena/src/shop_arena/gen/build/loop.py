...
Wires :class:`harness.PlanExecLoopConfig` against ``shop_arena.gen``'s prompts
...
from shop_arena.gen.build.sidecar import (
    SidecarHandle,
    parse_port_from_env,
    sidecar_lifecycle,
)
...
from shop_arena.gen.build.verifiers._skills import (
...
class VerifiersFactory(Protocol):
    ...
    def __call__(
        ...
        *,
        ...
        clickstream: Path | None = ...,
        clickstream_max_sessions: int = ...,
        clickstream_store_id: str | None = ...,
    ...
    *after* the seed manifest is captured.
        ...
        *,
        ...
        Args:
            ...
            install_runner: Subprocess runner used to spawn ``pnpm install``
                ...
                :func:`shop_arena.gen.build.verifiers._subprocess.default_subprocess_runner`.
                Tests inject a stub.
    ...
    def run(self, ctx: StepContext) -> None:
        ...
        Raises:
            ...
            shop_arena.gen.build.sidecar.SidecarLifecycleError: ``hydrogen/.env``
        ...
        with self._sidecar_factory(argv=argv, port=port) as sidecar:
            verifiers = self._verifiers_factory(
                ...
                clickstream=resolve_clickstream_path(ctx.config),
                clickstream_max_sessions=ctx.config.clickstream_max_sessions,
                clickstream_store_id=ctx.config.clickstream_store_id,
...
def _default_runtime_factory(config: ShopGenConfig) -> AgentRuntime:
    ...
    For ``pi`` runs, resolves the workspace-pinned ``pi-playwright``
    skill and passes it explicitly so nested visual verifier iterations
    can rely on the browser skill even though ``PiRuntime`` disables
    ...
    if config.runtime == "pi":
        ...
        if skill_dir is None:
            _log.warning(
                "pi-playwright skill not found for shop-gen runtime; visual browser "
                "iterations may fail unless the runtime can resolve browser tooling itself.",
            )
        else:
            kwargs["skill_paths"] = [skill_dir]
            _log.info("using pi-playwright skill for shop-gen runtime: %s", skill_dir)
...
def default_verifiers_factory(
    *,
    ...
    clickstream_max_sessions: int = DEFAULT_CLICKSTREAM_MAX_SESSIONS,
    ...
    is :data:`~shop_arena.gen.config.DEFAULT_JUDGES` (every known judge).
    ...
    Args:
        ...
        judges: Set of LLM-judge verifier names to construct (spec
            ...
            :data:`~shop_arena.gen.config.DEFAULT_JUDGES`.
        seed_dirs: Seed (shop-manual) directories whose
            ``artifact/evidence/`` trees hold the reference screenshots
            the ``visual_fidelity`` judge compares against. When empty
            (the default) the ``visual_fidelity`` judge is omitted even
            if requested — it has no ground truth without a seed.
        ...
        clickstream: Resolved path to the clickstream CSV
            (:func:`~shop_arena.gen.config.resolve_clickstream_path`).
            When present *and* the Python ``playwright`` package + its
            Chromium are installed, the ``clickstream_replay`` verifier
            is appended; otherwise it is omitted (with a warning when a
            CSV was supplied but Chromium is missing).
        clickstream_max_sessions: Cap on sampled clickstream sessions.
            Forwarded to the ``clickstream_replay`` ctor.
        clickstream_store_id: Optional ``store_id`` filter forwarded to
            the ``clickstream_replay`` ctor.
    ...
    if "visual_judge" in judges:
        if is_playwright_skill_available():
            _log.info(
                "registering visual_judge verifier timeout=%.0fs retry_budget=%d "
                "max_concurrency=%d pass_threshold=%.2f",
                DEFAULT_VISUAL_JUDGE_TIMEOUT_S,
                visual_retry_budget,
                visual_judge_max_concurrency,
                visual_judge_pass_threshold,
            )
            verifiers.append(
                VisualJudgeVerifier(
                    data_dir=out_dir / _DATA_DIR,
    ...
    if "visual_fidelity" in judges:
        if not seed_dirs:
            _log.warning(
                "`visual_fidelity` skipped: no seed directories were provided, "
                "so there is no source storefront to compare against.",
            )
        else:
            _log.info(
                "registering visual_fidelity verifier seeds=%d retry_budget=%d "
                "pass_threshold=%.2f",
                len(seed_dirs),
                visual_retry_budget,
                visual_judge_pass_threshold,
    ...
    # `clickstream_replay` is opt-in: registered only when a clickstream
    # CSV was resolved. It drives `playwright.sync_api` directly, so it
    # is additionally gated behind the Python-Playwright + Chromium probe
    # (warn + omit when the CSV is present but Chromium is missing, like
    # the `visual_judge` skill gate).
    if clickstream is not None:
        if is_python_playwright_available():
            _log.info(
                "registering clickstream_replay verifier csv=%s max_sessions=%d store_id=%s",
                clickstream,
                clickstream_max_sessions,
                clickstream_store_id,
            )
            verifiers.append(
                ClickstreamReplayVerifier(
                    ...
                    dev_server_factory=factory,
                    app_dir=app_dir,
                    max_sessions=clickstream_max_sessions,
                    store_id=clickstream_store_id,
                    retry_budget=visual_retry_budget,
                ),
            )
        else:
            _log.warning(
                "`clickstream_replay` skipped: Python Playwright/Chromium not available. "
                "Install with `uv run playwright install chromium` to enable.",
            )
