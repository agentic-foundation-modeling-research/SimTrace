...
- [ ] gen_homepage     — render hero, featured collections, and any promo banner from `homepage.section_types` and `artifact/manual/parts/homepage.md`; bind the hero to the `homepage.hero` Storefront field and banners to `homepage.banners`, plus live Storefront API queries [priority: 7]
