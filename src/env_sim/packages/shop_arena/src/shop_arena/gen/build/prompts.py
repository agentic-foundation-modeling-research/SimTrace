...
_VISUAL_FIDELITY_JUDGE_FILE: Final[str] = "visual_fidelity_judge.md"
...
@cache
def load_visual_fidelity_judge_prompt() -> str:
    """Return the ``str.format()`` template for the ``visual_fidelity`` verifier.

    The template carries six slots the verifier renders before the
    vision call:

    * ``{task_id}`` — selected task id at dispatch time.
    * ``{buckets}`` — comma-joined page-bucket names under review.
    * ``{route_list}`` — markdown bullet list of the captured routes.
    * ``{reference_count}`` — number of reference (source-storefront)
      images sent, which lead the image sequence.
    * ``{generated_count}`` — number of generated-storefront images sent,
      which follow the reference images.
    * ``{verdict_schema}`` — JSON schema the model must emit.

    Returns:
        The template body, terminated by a single newline.

    Raises:
        FileNotFoundError: ``visual_fidelity_judge.md`` is missing.
    """
    return _read_prompt_file(_VISUAL_FIDELITY_JUDGE_FILE)
...
__all__ = [
    ...
    "load_visual_fidelity_judge_prompt",
