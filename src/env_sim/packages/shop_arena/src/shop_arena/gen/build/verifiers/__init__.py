...
driver. The harness owns dispatch + telemetry; ``shop_arena.gen`` owns every
...
* :class:`ClickstreamReplayVerifier` — replays real user clickstream
  trajectories against the generated twin with Playwright and gates on
  strict reproduction of each session's actions. Opt-in (registered
  only when a clickstream CSV is supplied); applies to ``gen_navigation``
  and ``visual_fix`` by default.
...
from shop_arena.gen.build.verifiers.clickstream_replay import ClickstreamReplayVerifier
...
from shop_arena.gen.build.verifiers.nav_coverage import NavCoverageVerifier
from shop_arena.gen.build.verifiers.navigation_primitive_usage import (
    NavigationPrimitiveUsageVerifier,
)
from shop_arena.gen.build.verifiers.no_brand_leak import NoBrandLeakVerifier
from shop_arena.gen.build.verifiers.quality_judge import QualityJudgeVerifier
...
from shop_arena.gen.build.verifiers.visual_judge import (
