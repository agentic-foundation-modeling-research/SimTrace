...
class SynthNavigationStep:
    ...
    def __init__(
        ...
        *,
        ...
        collections_step_id: str = _COLLECTIONS_UPSTREAM_ID,
    ) -> None:
        ...
        Args:
            ...
            collections_step_id: Upstream step id that writes the cached
                ``.shop_gen/stage_cache/collections.json``. Defaults to
                ``synth_collections`` (LLM catalog mode); the verbatim
                ingest mode passes ``ingest_catalog``. The cached-file
                path ``run`` reads is identical in both modes.
        """
