...
:class:`~shop_arena.gen.data_synth.schema.Product` records, and emits the
...
historically run the :mod:`shop_arena.gen.brands.allowlist` scanner over
...
* ``outputs``: the six ``data/*.json`` files listed above. The
  ...
  :class:`~shop_arena.gen.data_synth.images.GenImagesStep` and **not**
  declared here.
...
from shop_arena.gen.brands.allowlist import Allowlist, Hit, load_allowlist, scan
from shop_arena.gen.config import CatalogSource
...
_UPSTREAM_NAVIGATION_INGEST: Final[str] = "ingest_navigation"
...
_UPSTREAM_IDS_INGEST: Final[tuple[str, ...]] = (
    _UPSTREAM_IDENTITY,
    _UPSTREAM_STORE,
    _UPSTREAM_PAGES,
    _UPSTREAM_POLICIES,
    _UPSTREAM_NAVIGATION_INGEST,
)
"""Upstreams in verbatim ingest mode.

``ingest_catalog`` owns ``data/{products,collections}.json``, and
``ingest_navigation`` owns the navigation cache (mapped verbatim from the
storefront's captured menus), so ``assemble_data`` no longer depends on the
LLM catalog/image steps or ``synth_navigation`` — only the prose producers
plus ``ingest_navigation`` remain.
"""
...
_OUT_FILES_INGEST: Final[tuple[Path, ...]] = (
    _OUT_STORE,
    _OUT_PAGES,
    _OUT_POLICIES,
    _OUT_NAVIGATION,
)
"""Files ``assemble_data`` owns in ingest mode (prose only).

``data/{products,collections}.json`` are owned by ``ingest_catalog``.
"""
...
def assemble_prose_records(
    *,
    ...
    """Assemble the four prose records for verbatim-ingest runs.

    The reduced counterpart of :func:`assemble_records` used when
    ``catalog_source="ingest"``: ``ingest_catalog`` owns
    ``data/{products,collections}.json``, so ``assemble_data`` only
    validates and stamps the store / pages / policies / navigation
    records. Pure — no I/O.
    ...
    Args:
        ...
        pages_payload: Decoded ``.shop_gen/stage_cache/pages.json``.
        policies_payload: Decoded ``.shop_gen/stage_cache/policies.json``.
        navigation_payload: Decoded ``.shop_gen/stage_cache/navigation.json``.

    Returns:
        :class:`AssembledProse` carrying the four validated records.

    Raises:
        StageSynthError: A payload is missing ``name`` or fails schema
            validation.
    """
    return AssembledProse(
        store=_assemble_store(identity=identity, store_payload=store_payload),
        pages=_validate_pages(pages_payload),
        policies=_validate_policies(policies_payload),
        navigation=_validate_navigation(navigation_payload),
...
class AssembledProse:
    """Bundle of the four prose records assembled in ingest mode.
    ...
    __slots__ = ("navigation", "pages", "policies", "store")
...
class AssembleDataStep:
    ...
    published ``data/*.json`` files.

    In the default ``synth`` mode the step owns all six files. In
    ``ingest`` mode ``ingest_catalog`` owns ``data/products.json`` and
    ``data/collections.json``, so this step drops those two outputs and
    the LLM catalog/image upstreams and writes only the four prose files
    (store / pages / policies / navigation).

    Attributes:
        id: Step id (``assemble_data``).
        phase: ``data_synth``.
        inputs: One :class:`StepInput` per upstream Phase 2 step (reduced
            in ingest mode).
        outputs: The owned ``data/*.json`` paths (six in synth mode, four
            in ingest mode).
        depends_on: Same upstream ids as :attr:`inputs`.
        version: Bumped when the assembly behaviour changes (spec §5.7.1).
    """

    def __init__(self, *, catalog_source: CatalogSource = "synth") -> None:
        """Build the step, wiring upstreams / outputs to the catalog mode.

        Args:
            catalog_source: ``synth`` (default) owns all six ``data/*.json``
                files and depends on the LLM catalog/image steps; ``ingest``
                owns only the four prose files and depends only on the prose
                producers plus ``synth_navigation``.
        """
        self._catalog_source: CatalogSource = catalog_source
        upstream_ids = _UPSTREAM_IDS_INGEST if catalog_source == "ingest" else _UPSTREAM_IDS
        out_files = _OUT_FILES_INGEST if catalog_source == "ingest" else _OUT_FILES
        self.id: str = _STEP_ID
        self.phase: str = _PHASE
        self.inputs: list[InputRef] = [StepInput(step_id=sid) for sid in upstream_ids]
        self.outputs: list[Path] = list(out_files)
        self.depends_on: list[str] = list(upstream_ids)
        self.version: int = _STEP_VERSION

    def run(self, ctx: StepContext) -> None:
        """Assemble the owned ``data/*.json`` files.
        ...
        """
        if self._catalog_source == "ingest":
            self._run_ingest(ctx)
            return
    ...
    def _run_ingest(self, ctx: StepContext) -> None:
        """Assemble only the four prose files (verbatim-ingest mode).

        ``ingest_catalog`` already wrote ``data/products.json`` and
        ``data/collections.json``; this path reads the prose caches and
        stamps the store / pages / policies / navigation records.

        Args:
            ctx: Execution context. ``ctx.runtime`` is unused.

        Raises:
            FileNotFoundError: Any prose cache is missing.
            StageSynthError: A cache is malformed or fails validation.
        """
        identity = _load_json_object(ctx.out_dir / _IN_IDENTITY, label="identity.json")
        store_payload = _load_json_object(ctx.out_dir / _IN_STORE, label="store cache")
        pages_payload = _load_json_array(ctx.out_dir / _IN_PAGES, label="pages cache")
        ...
        _write_prose_outputs(ctx.out_dir, prose)
...
def _write_prose_outputs(out_dir: Path, prose: AssembledProse) -> None:
    """Serialise the four prose records to ``<out_dir>/data/*.json`` (ingest mode).

    Mirrors :func:`_write_outputs` for the store / pages / policies /
    navigation files (including the ``dataset_version`` drop) while leaving
    ``products.json`` / ``collections.json`` to ``ingest_catalog``.
    """
    data_dir = out_dir / _DATA_DIR
    data_dir.mkdir(parents=True, exist_ok=True)
    store_payload = prose.store.model_dump(mode="json")
    # See _write_outputs: shop_backend treats dataset_version as
    # ``string | undefined``, not nullable — drop the key when unset.
    if store_payload.get("dataset_version") is None:
        store_payload.pop("dataset_version", None)
    _write_json(out_dir / _OUT_STORE, store_payload)
    _write_json(
        out_dir / _OUT_PAGES,
        [p.model_dump(mode="json") for p in prose.pages],
    )
    _write_json(
        out_dir / _OUT_POLICIES,
        [p.model_dump(mode="json") for p in prose.policies],
    )
    _write_json(out_dir / _OUT_NAVIGATION, prose.navigation.model_dump(mode="json"))
...
def _load_details_by_handle(
    *,
    ...
    Mirrors the loader in :mod:`shop_arena.gen.data_synth.images`; duplicated
