...
class ProductImage(BaseModel):
    ...
    Attributes:
        ...
        width: Pixel width, or ``None`` when the feed omits it (collection
            hero images ship no dimensions).
        height: Pixel height, or ``None`` (see ``width``).
...
class Homepage(BaseModel):
    """Site-level editorial imagery for ``homepage.json`` (spec §8.1.1).

    Reuses the original storefront's non-product hero and promo-banner
    imagery verbatim so the twin's homepage matches the source site. Both
    fields hold captured absolute CDN URLs (hotlinked, not downloaded);
    ``shop_backend`` passes such absolute ``src`` values through untouched.

    Attributes:
        hero: The single most prominent above-the-fold image, or ``None``
            when the capture found none.
        banners: Ordered promo banners below the hero (possibly empty).
    """

    model_config = _CLOSED_FROZEN

    hero: ProductImage | None
    banners: list[ProductImage]
