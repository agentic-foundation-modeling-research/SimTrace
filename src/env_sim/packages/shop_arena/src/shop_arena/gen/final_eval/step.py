...
* :class:`~shop_arena.gen.final_eval.playwright_smoke.DevServerFactory` — boots
  the dev server.
* :class:`~shop_arena.gen.final_eval.playwright_smoke.BrowserDriver` — walks
  the smoke flow and writes screenshots.
* :class:`SmokeRunner` — drives the dev server + browser; defaults to
  :func:`shop_arena.gen.final_eval.playwright_smoke.run_playwright_smoke`.
...
from shop_arena.gen.build.verifiers._task_routes import SWEEP_CAPS, BucketCaps
...
_ITERS_DIR: Final[Path] = Path("runs") / "build" / "iters"
"""Run-relative build-loop iters directory the clickstream rollup scans."""
        ...
        *,
        ...
        Args:
            ...
            visual_sweep_runner: Drives the all-pages visual sweep
                ...
                :func:`~shop_arena.gen.final_eval.visual_sweep.run_visual_sweep`.
...
def run_final_eval(
    *,
    ...
    Returns:
        ...
        * ``clickstream`` — advisory cross-iteration rollup of the
          ``clickstream_replay`` verifier (§ build loop):
          ``{ran, sessions_sampled, dropped, iterations[], final}``.
          ``ran`` is ``false`` when no CSV was supplied or the verifier
          never replayed a session.
    """
    ...
    # The ``clickstream`` subtree is a cross-iteration rollup of the
    # ``clickstream_replay`` verifier's per-iteration telemetry. Like
    # ``visual`` it is **advisory** and does not gate ``ok``.
    clickstream_payload = build_clickstream_subtree(out_dir / _ITERS_DIR)
    ...
    return {
        ...
        "clickstream": clickstream_payload,
    }
...
def _parse_judge_output(raw: str) -> tuple[str, str] | None:
    ...
    (:mod:`shop_arena.gen.build.verifiers._judge`): forgive surrounding
