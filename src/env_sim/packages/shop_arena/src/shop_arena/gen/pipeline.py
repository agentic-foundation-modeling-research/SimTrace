...
from shop_arena.gen.config import CatalogSource, ShopGenConfig, ShopGenResult
...
def list_steps() -> dict[str, tuple[str, ...]]:
    ...
    single_seed_ingest = _build_registry_from_branch(multi_seed=False, catalog_source="ingest")

    grouped: dict[str, list[str]] = {phase: [] for phase in PHASES}
    seen: set[str] = set()
    for reg in (single_seed, multi_seed, single_seed_ingest):
        for step in reg.all():
            if step.id in seen:
                continue
            seen.add(step.id)
...
def _build_registry(config: ShopGenConfig) -> Registry:
    ...
    registered. ``config.catalog_source`` selects the Phase 2 catalog
    branch (LLM synth vs verbatim ingest).
    """
    return _build_registry_from_branch(
...
def _build_registry_from_branch(
    *,
    ...
    catalog_source: CatalogSource = "synth",
) -> Registry:
    ...
    seed_dir = None if config is None or len(config.seeds) != 1 else config.seeds[0]
...
def _register_data_synth(
    ...
    *,
    ...
    The prose steps ``synth_identity`` / ``synth_store`` / ``synth_pages`` /
    ``synth_policies`` and the terminal ``assemble_data`` register in both
    catalog modes. The catalog itself branches on ``catalog_source``:

    * ``synth`` (default) registers the LLM catalog chain
      ``synth_collections`` → ``synth_product_skeletons`` →
      ``synth_product_details`` → ``synth_alt_text`` → ``gen_images`` and
      wires ``synth_navigation`` to ``synth_collections``.
    * ``ingest`` registers a single ``ingest_catalog`` step that transforms
      the seed's real prefetch feeds verbatim, wires ``synth_navigation``
      to ``ingest_catalog``, and omits the five LLM catalog/image steps.
      ``assemble_data`` is put in ingest mode so it stops owning
      products/collections (see :class:`AssembleDataStep`).
    ...
    Args:
        ...
        stats_aware_manual_step_ids: Like ``manual_step_ids`` but also
            ...
            staleness correctly. Unused in ingest mode.
        catalog_source: Phase 2 catalog branch selector.
        seed_dir: The single seed directory whose prefetch feeds
            ``ingest_catalog`` reads. ``None`` in the listing branch or
            for multi-seed runs; ingest mode is only reachable for a
            single seed (enforced by ``ShopGenConfig``).
    """
    ...
    # Site-level hero/banner ingest runs in both catalog modes: the editorial
    # imagery is reused from the real store regardless of how the catalog was
    # produced. Best-effort — an absent capture yields an empty homepage.
    registry.register(IngestHomepageStep(seed_dir=seed_dir))
    if catalog_source == "ingest":
        registry.register(IngestCatalogStep(seed_dir=seed_dir))
        registry.register(IngestNavigationStep(seed_dir=seed_dir))
        registry.register(AssembleDataStep(catalog_source="ingest"))
        return
    registry.register(
