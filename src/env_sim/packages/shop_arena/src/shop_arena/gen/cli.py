...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--catalog-source",
        choices=("synth", "ingest"),
        default=DEFAULT_CATALOG_SOURCE,
        help=(
            f"Catalog source. Default: {DEFAULT_CATALOG_SOURCE!r}. 'synth' "
            "LLM-synthesizes the catalog from the merged manual; 'ingest' "
            "transforms the seed's real prefetch feeds "
            "(artifact/prefetch/{products,collections}.json) verbatim and "
            "skips the LLM catalog + image-generation steps (single seed only)."
        ),
    )
    parser.add_argument(
        "--image-model",
    ...
    parser.add_argument(
        ...
        metavar="PATH",
        help=(
            "Path to a clickstream CSV replayed by the `clickstream_replay` "
            "verifier (opt-in). Defaults to auto-discovering "
            "<seed>/clickstream.csv for a single-seed run; when neither is "
            "present the verifier is omitted. Best signal in twin "
            "(--catalog-source ingest) mode where the twin preserves live-store handles."
        ),
    )
    parser.add_argument(
        ...
        metavar="N",
        help=(
            "Cap on clickstream sessions replayed (deterministic: first N by "
            f"sorted session_id). Strictly positive. Default: {DEFAULT_CLICKSTREAM_MAX_SESSIONS}."
        ),
    )
    parser.add_argument(
        "--clickstream-store-id",
        default=None,
        metavar="ID",
        help=(
            "When set, replay only clickstream rows whose `store_id` matches. "
            "Default: replay every row in the CSV."
        ),
    )
...
def _build_config(args: argparse.Namespace) -> ShopGenConfig:
    ...
    catalog_source: CatalogSource = args.catalog_source
    ...
    return ShopGenConfig(
        ...
        judges=judges,
        clickstream=args.clickstream,
        clickstream_max_sessions=args.clickstream_max_sessions,
        clickstream_store_id=args.clickstream_store_id,
    )
