...
CatalogSource = Literal["synth", "ingest"]
"""Source of the product catalog written to ``data/{products,collections}.json``.

``synth`` (default) LLM-synthesizes the catalog from the merged manual.
``ingest`` transforms the real Shopify storefront feeds captured by
``shop-explore`` (``<seed>/artifact/prefetch/{products,collections}.json``)
verbatim into the SandboxShop schema and derives collection membership
heuristically. ``ingest`` requires exactly one seed and skips the five LLM
catalog/image steps (see
:class:`shop_arena.gen.data_synth.ingest.IngestCatalogStep`).
"""
...
def resolve_clickstream_path(config: ShopGenConfig) -> Path | None:
    """Resolve the clickstream CSV to replay, or ``None`` when absent.

    Resolution order (spec: convention next to the seed, with an
    explicit override):

    1. :attr:`ShopGenConfig.clickstream` when set (already validated to
       be an existing file).
    2. Otherwise, for a single-seed run, ``<seed>/clickstream.csv`` when
       that file exists.
    3. Otherwise ``None`` — the ``clickstream_replay`` verifier is
       omitted and the build proceeds without it.

    Args:
        config: The run configuration.

    Returns:
        Absolute path to the CSV to replay, or ``None``.
    """
    if config.clickstream is not None:
        return config.clickstream
    if len(config.seeds) != 1:
        return None
    candidate = config.seeds[0] / _CLICKSTREAM_CONVENTION
...
DEFAULT_CATALOG_SOURCE: Final[CatalogSource] = "synth"
"""Default catalog source: LLM synthesis (see :data:`CatalogSource`)."""
...
DEFAULT_CLICKSTREAM_MAX_SESSIONS: Final[int] = 15
"""Default cap on clickstream sessions replayed by ``clickstream_replay``.

Deterministic sample: the first N sessions by sorted ``session_id`` are
replayed; the rest are dropped (and the count logged). Configurable via
``--clickstream-max-sessions``.
"""

_CLICKSTREAM_CONVENTION: Final[str] = "clickstream.csv"
"""Conventional CSV filename auto-discovered next to a single seed."""

KNOWN_JUDGES: Final[frozenset[str]] = frozenset(
    {"visual_judge", "quality_judge", "cross_task_consistency", "visual_fidelity"},
)
...
class ShopGenConfig(BaseModel):
    ...
    Attributes:
        ...
        catalog: Scale knobs (see :class:`CatalogConfig`). Ignored when
            ``catalog_source="ingest"`` (the catalog size is the real feed's).
        catalog_source: ``synth`` (default) LLM-synthesizes the catalog;
            ``ingest`` transforms the seed's real prefetch feeds verbatim
            (see :data:`CatalogSource`). ``ingest`` requires exactly one seed.
        ...
        final_eval_visual_max_concurrency: Page-bucket fan-out worker count
            ...
            positive.
        clickstream: Explicit override path to the clickstream CSV
            replayed by the ``clickstream_replay`` verifier. ``None``
            (default) auto-discovers ``<seed>/clickstream.csv`` for a
            single-seed run via :func:`resolve_clickstream_path`. When
            set it must point at an existing file (validated at
            construction). Best signal in twin (``catalog_source=
            "ingest"``) mode where the twin preserves live-store handles.
        clickstream_max_sessions: Cap on the number of clickstream
            sessions replayed (deterministic: first N by sorted
            ``session_id``). Defaults to
            :data:`DEFAULT_CLICKSTREAM_MAX_SESSIONS`. Strictly positive.
        clickstream_store_id: When set, replay only clickstream rows
            whose ``store_id`` equals this value. ``None`` (default)
            replays every row in the CSV.
    """
    ...
    catalog_source: CatalogSource = DEFAULT_CATALOG_SOURCE
    ...
    clickstream: Path | None = None
    clickstream_max_sessions: int = Field(
        default=DEFAULT_CLICKSTREAM_MAX_SESSIONS,
        gt=0,
    )
    clickstream_store_id: str | None = None
    ...
    @field_validator("clickstream", mode="before")
    @classmethod
    def _coerce_clickstream(cls, value: object) -> object:
        """Make a supplied clickstream path absolute against the cwd.

        Mirrors :meth:`_coerce_seeds`: a relative ``--clickstream`` path
        is anchored at construction time so it is not later re-rooted
        under ``out_dir``. ``None`` (auto-discovery) passes through.
        """
        if isinstance(value, (str, Path)):
            return _to_absolute(Path(value))
        return value
    ...
    @model_validator(mode="after")
    def _validate_catalog_source(self) -> ShopGenConfig:
        """Require a single seed for verbatim catalog ingest.

        ``catalog_source="ingest"`` transforms exactly one storefront's
        prefetch feeds; there is no defined merge of two real catalogs, so
        multi-seed ingest is rejected at construction time. Feed presence is
        not checked here (no I/O) — the ingest step raises a clear error if a
        feed is missing.
        """
        if self.catalog_source == "ingest" and len(self.seeds) != 1:
            raise ValueError(
                "catalog_source='ingest' requires exactly one seed, "
                f"got {len(self.seeds)}",
            )
        return self
    ...
    @model_validator(mode="after")
    def _validate_clickstream_path(self) -> ShopGenConfig:
        """Reject an explicit ``clickstream`` path that is not a file.

        Auto-discovery (``clickstream is None``) is resolved later by
        :func:`resolve_clickstream_path`, which tolerates a missing
        convention file; an *explicitly* supplied path that does not
        point at a readable file is a user error and fails fast.
        """
        if self.clickstream is not None and not self.clickstream.is_file():
            raise ValueError(
                f"clickstream must be an existing file, got: {self.clickstream}",
            )
        return self
