"""Unit tests for :mod:`shop_arena.explore.prefetch`.
...
from shop_arena.explore.prefetch import (
    DEFAULT_USER_AGENT,
    PRODUCTS_PAGE_LIMIT,
    PrefetchResult,
...
from shop_arena.explore.stats import compute as compute_stats
...
``_fetch_products_paginated`` in ``shop_arena.explore.prefetch.runner``."""
...
# --------------------------------------------------------------------------- #
# products_collections.json — per-collection membership capture
# --------------------------------------------------------------------------- #


def _make_products_with_handles(handles: list[str], *, start: int) -> list[dict[str, Any]]:
    """Build product dicts carrying ``handle`` (what membership capture reads)."""
    return [
        {
            "id": start + i,
...
def _stub_collection_page(
    ...
    *,
    ...
    """Stub one page of ``/collections/{handle}/products.json``."""
    mock.get(
        f"{BASE_URL}/collections/{handle}/products.json",
        params={"page": str(page), "limit": str(PRODUCTS_PAGE_LIMIT)},
    ).mock(return_value=_ok(json.dumps({"products": products}), content_type="application/json"))


@respx.mock
def test_run_captures_products_collections_membership(tmp_path: Path) -> None:
    """Each collection's products feed is walked into products_collections.json."""
    _stub_storefront(respx.mock)
    respx.mock.get(f"{BASE_URL}/collections.json", params={"limit": "50"}).mock(
        return_value=_ok(
            json.dumps({"collections": [{"handle": "summer"}, {"handle": "winter"}]}),
            content_type="application/json",
        )
    )
    _stub_collection_page(
        respx.mock,
        handle="summer",
        page=1,
        products=_make_products_with_handles(["linen-shirt", "straw-hat"], start=1),
    )
    _stub_collection_page(
        respx.mock,
        handle="winter",
        page=1,
        products=_make_products_with_handles(["wool-coat"], start=100),
    )

    result = run(BASE_URL, dest_dir=tmp_path, rate_limit_ms=0)

    membership: Any = json.loads(
        (tmp_path / "products_collections.json").read_text(encoding="utf-8"),
    )
    assert membership == {"summer": ["linen-shirt", "straw-hat"], "winter": ["wool-coat"]}

    # Each collection page is its own entry; the first contributing entry marks
    # the artifact origin, later ones have saved_to=None.
    coll_entries = [e for e in result.entries if e.path.startswith("/collections/")]
    assert [e.path for e in coll_entries] == [
        f"/collections/summer/products.json?page=1&limit={PRODUCTS_PAGE_LIMIT}",
        f"/collections/winter/products.json?page=1&limit={PRODUCTS_PAGE_LIMIT}",
    ]
    assert coll_entries[0].saved_to == "products_collections.json"
    assert coll_entries[1].saved_to is None


@respx.mock
def test_run_paginates_collection_products_until_short_page(tmp_path: Path) -> None:
    """A collection's membership walk merges pages and stops on the short page."""
    _stub_storefront(respx.mock)
    respx.mock.get(f"{BASE_URL}/collections.json", params={"limit": "50"}).mock(
        return_value=_ok(
            json.dumps({"collections": [{"handle": "big"}]}),
            content_type="application/json",
        )
    )
    page_one_handles = [f"p{i}" for i in range(PRODUCTS_PAGE_LIMIT)]
    page_two_handles = ["last-a", "last-b"]
    _stub_collection_page(
        respx.mock,
        handle="big",
        page=1,
        products=_make_products_with_handles(page_one_handles, start=1),
    )
    _stub_collection_page(
        respx.mock,
        handle="big",
        page=2,
        products=_make_products_with_handles(page_two_handles, start=10_000),
    )
    ...
    assert membership["big"] == page_one_handles + page_two_handles

    # No third page — the short page two ended pagination for this collection.
    fetched = [call.request.url.path for call in respx.mock.calls]
    assert fetched.count("/collections/big/products.json") == 2


@respx.mock
def test_run_skips_products_collections_when_no_collections(tmp_path: Path) -> None:
    """No collections → no products_collections.json artifact is written."""
    _stub_storefront(respx.mock)  # default collections.json is empty

    run(BASE_URL, dest_dir=tmp_path, rate_limit_ms=0)

    assert not (tmp_path / "products_collections.json").exists()
