...
def test_catalog_source_flag_propagates_to_config(
    tmp_path: Path,
    captured_run: dict[str, Any],
) -> None:
    """``--catalog-source ingest`` threads into ``ShopGenConfig``."""
    seed = _make_seed(tmp_path)
    rc = main([str(seed), "--out-dir", str(tmp_path / "out"), "--catalog-source", "ingest"])
    assert rc == EXIT_OK
    assert captured_run["config"].catalog_source == "ingest"


def test_catalog_source_defaults_to_synth(
    tmp_path: Path,
    captured_run: dict[str, Any],
) -> None:
    seed = _make_seed(tmp_path)
    ...
    assert rc == EXIT_OK
    assert captured_run["config"].catalog_source == "synth"


def test_multi_seed_ingest_exits_config_error(
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
    """A multi-seed ``--catalog-source ingest`` run trips the single-seed validator."""
    seed_a = _make_seed(tmp_path, "seed-a")
    seed_b = _make_seed(tmp_path, "seed-b")
    rc = main(
        [str(seed_a), str(seed_b), "--name", "acme", "--catalog-source", "ingest"],
    )
    assert rc == EXIT_CONFIG
    assert "invalid configuration" in capsys.readouterr().err
