...
from shop_arena.gen.data_validation.hosting_check import _pick_image
...
# --------------------------------------------------------------------------- #
# _pick_image — image-source selection (pure, no backend)
# --------------------------------------------------------------------------- #


def test_pick_image_accepts_relative_src_with_query_suffix() -> None:
    """A relative src is recognised by extension despite a ``?v=`` suffix."""
    products = [{"handle": "p1", "images": [{"src": "photo.jpg?v=9"}]}]
    assert _pick_image(products) == ("p1", "photo.jpg?v=9")


def test_pick_image_returns_none_for_non_image_relative_src() -> None:
    """A relative src without a recognised image extension is skipped."""
    products = [{"handle": "p1", "images": [{"src": "notes.txt"}]}]
    assert _pick_image(products) is None
...
@pytest.mark.skipif(not _backend_available(), reason=_BACKEND_REASON)
def test_run_hosting_checks_passes_with_remote_cdn_images(tmp_path: Path) -> None:
    """Verbatim absolute CDN image srcs pass ``image_get`` via GraphQL passthrough.

    Ingested catalogs keep remote srcs and ship no local ``images/`` tree, so
    the image check must verify the ``featuredImage.url`` passthrough rather
    than fetching ``/images/<src>``.
    """
    data_dir = _materialise_data_dir(tmp_path)
    shutil.rmtree(data_dir / "images")
    products = json.loads((data_dir / "products.json").read_text(encoding="utf-8"))
    for product in products:
        for position, image in enumerate(product.get("images", []) or [], start=1):
            image["src"] = f"https://cdn.shopify.com/s/files/1/remote-{position}.jpg?v=123"
    (data_dir / "products.json").write_text(json.dumps(products), encoding="utf-8")

    verdict = run_hosting_checks(data_dir)

    assert verdict["ok"] is True
    image_check = next(c for c in verdict["checks"] if c["name"] == "image_get")
    assert image_check["ok"] is True
    assert "passthrough" in image_check["detail"]
