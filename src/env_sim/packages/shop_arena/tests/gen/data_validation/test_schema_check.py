...
# The step also declares ``homepage.json`` as a file input, but it is optional
# (validated only when present) so it is absent from the required-file fixture.
_STEP_INPUT_FILES: tuple[str, ...] = (*_FIXTURE_FILES, "homepage.json")
...
def test_step_metadata() -> None:
    ...
    assert step.depends_on == ["assemble_data", "ingest_homepage"]
    # Verdict is in-memory (spec §5.4): no declared output files.
    assert step.outputs == []
    assert step.version >= 1


def test_step_inputs_cover_all_data_files() -> None:
    """Inputs must include the upstream step refs + every ``data/*.json`` file."""
    step = ValidateSchemaStep()
    ...
    assert [ref.step_id for ref in step_inputs] == ["assemble_data", "ingest_homepage"]
    assert sorted(ref.path.name for ref in file_inputs) == sorted(_STEP_INPUT_FILES)
