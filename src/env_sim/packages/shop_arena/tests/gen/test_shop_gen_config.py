...
def test_known_judges_matches_spec_section_5_5() -> None:
    """Spec §5.5: closed set of LLM judges (plus the ported ``visual_fidelity``)."""
    assert (
        frozenset(
...
def test_shop_gen_config_default_catalog_source_is_synth(tmp_path: Path) -> None:
    seed = _seed(tmp_path)
    assert ShopGenConfig(seeds=[seed]).catalog_source == "synth"


def test_shop_gen_config_single_seed_ingest_constructs(tmp_path: Path) -> None:
    seed = _seed(tmp_path)
    cfg = ShopGenConfig(seeds=[seed], catalog_source="ingest")
    assert cfg.catalog_source == "ingest"


def test_shop_gen_config_multi_seed_ingest_rejected(tmp_path: Path) -> None:
    seeds = [_seed(tmp_path, "seed-a"), _seed(tmp_path, "seed-b")]
    with pytest.raises(ValidationError, match="requires exactly one seed"):
        ShopGenConfig(seeds=seeds, catalog_source="ingest")


def test_shop_gen_config_rejects_unknown_catalog_source(tmp_path: Path) -> None:
    seed = _seed(tmp_path)
    with pytest.raises(ValidationError):
        ShopGenConfig(seeds=[seed], catalog_source="bogus")  # type: ignore[arg-type]
