...
* :class:`shop_arena.gen.data_synth.assemble.AssembleDataStep` emits a
  complete ``data/`` directory whose six published files round-trip
  through the closed pydantic mirrors from
  :mod:`shop_arena.gen.data_synth.schema` (SC1).

Phase 1 is collapsed to the single-seed shortcut
(:class:`shop_arena.gen.manual_merge.copy_seed.CopySeedManualStep`) so the
...
registered DAG via :func:`shop_arena.gen.steps.runner.run_pipeline` directly,
...
_EXPECTED_STEP_IDS: frozenset[str] = frozenset(
    {
        ...
        "synth_navigation",
        "assemble_data",
        # Site-level hero/banner ingest (best-effort — writes an empty
        # ``homepage.json`` here since the seed carries no hero capture).
        "ingest_homepage",
...
def _assert_data_files_present(data_dir: Path) -> None:
    """Assert every published ``data/*.json`` file exists."""
    expected = {
        "store.json",
        "products.json",
        "collections.json",
...
def test_phase2_single_seed_end_to_end(tmp_path: Path) -> None:
    ...
    * The brand-leak scrub is silent: ``data/`` carries only allowlisted
    ...
    # its own integration test (tests/gen/data_validation/
    # test_hosting_check.py); the build env-setup steps (T5.1) operate
    # against the vendored Hydrogen template and are exercised in
    # their own unit tests (tests/gen/build/test_env.py); the
    # ``start_sidecar`` step (T5.2) boots ``shop-backend`` against
    # the assembled data and is exercised in its own unit test
    # (tests/gen/build/test_sidecar.py). Skip them all here so
