"""Unit + integration tests for :mod:`shop_arena.gen.build.loop`.
...
from shop_arena.gen.build.loop import (
...
def _empty_verifiers_factory(
    *,
    ...
    del out_dir, sidecar, judges, seed_dirs, visual_retry_budget
...
def test_step_forwards_visual_retry_budget_to_verifiers_factory(
    ...
    def _verifiers_factory(
        *,
        ...
        del out_dir, sidecar, judges, seed_dirs
...
def test_step_forwards_judges_to_verifiers_factory(
    ...
    def _verifiers_factory(
        *,
        ...
        del out_dir, sidecar, seed_dirs, visual_retry_budget
        del visual_judge_pass_threshold, visual_judge_max_concurrency, template_id
    ...
    * the loop runner captures the :class:`PlanExecLoopConfig` rather
    ...
    def _verifiers_factory(
        *,
        ...
        del out_dir, judges, seed_dirs, visual_retry_budget
        del visual_judge_pass_threshold, visual_judge_max_concurrency, template_id
...
def test_step_run_resets_run_dir_when_source_drifts(tmp_path: Path) -> None:
    ...
    (hydrogen + data, mixed with :data:`shop_arena.gen.build.env._CLONE_STEP_VERSION`)
...
_REPLAY_PLAN_MD: Final[str] = (
    "# Plan\n\n## Tasks\n- [x] gen_theme   [priority: 9]\n- [x] visual_fix   [priority: 2]\n"
)
"""Plan body the replay cassette emits — already-done so the executor loop drains immediately.

Includes the canonical ``visual_fix`` task as the lowest-priority
final bullet (spec §5.5.4) so the planner contract is satisfied.
"""
...
def test_default_verifiers_factory_returns_v01_set_when_skill_missing(
    ...
    with (
        patch("shop_arena.gen.build.loop.is_playwright_skill_available", return_value=False),
        caplog.at_level("WARNING", logger="shop_arena.gen.build.loop"),
    ):
...
def test_default_verifiers_factory_includes_visual_judge_when_skill_present(
    ...
    sidecar = _stub_handle()
    with (
        patch("shop_arena.gen.build.loop.is_playwright_skill_available", return_value=True),
        caplog.at_level("WARNING", logger="shop_arena.gen.build.loop"),
    ):
        verifiers = default_verifiers_factory(out_dir=out_dir, sidecar=sidecar)
...
def test_default_verifiers_factory_registers_visual_fidelity_with_seed_dirs(
    tmp_path: Path,
) -> None:
    """``visual_fidelity`` is included only when seed dirs are provided."""
    out_dir = tmp_path / "out"
    out_dir.mkdir()
    _materialise_workspace(out_dir)
    seed = tmp_path / "seed"
    ...
    with patch("shop_arena.gen.build.loop.is_playwright_skill_available", return_value=True):
        verifiers = default_verifiers_factory(
            out_dir=out_dir,
            sidecar=sidecar,
            judges=frozenset({"visual_fidelity"}),
            seed_dirs=(seed,),
        )

    assert any(isinstance(v, VisualFidelityVerifier) for v in verifiers)


def test_default_verifiers_factory_skips_visual_fidelity_without_seed_dirs(
    tmp_path: Path,
    caplog: pytest.LogCaptureFixture,
) -> None:
    """``visual_fidelity`` is dropped (with a warning) when no seed dirs are given."""
    out_dir = tmp_path / "out"
    out_dir.mkdir()
    _materialise_workspace(out_dir)
    ...
    assert not any(isinstance(v, VisualFidelityVerifier) for v in verifiers)
    assert "visual_fidelity` skipped" in caplog.text
...
# --------------------------------------------------------------------------- #
# Single-pass loop contract (visual_fix is a planner-emitted canonical task)
# --------------------------------------------------------------------------- #


_PLAN_WITH_VISUAL_FIX: Final[str] = (
    "# Plan\n\n## Tasks\n"
    "- [x] gen_theme — done [priority: 9]\n"
    "- [x] visual_fix — done [priority: 2]\n"
)
"""Compliant planner output — ``visual_fix`` is the lowest-priority bullet."""


def test_step_run_invokes_loop_once(tmp_path: Path) -> None:
    """The orchestrator runs the harness exactly once: ``visual_fix`` is a planner task.

    The earlier ``consolidate``-append fallback (T5.8) is retired —
    the canonical 8-task plan now ends with ``visual_fix`` and the
    harness drives it inline like any other task.
    """
    out_dir = tmp_path / "out"
    out_dir.mkdir()
    _materialise_workspace(out_dir)
    ...
    def _loop_runner(
        ...
        *,
        ...
        call_count["n"] += 1
        if call_count["n"] == 1:
            plan_path.write_text(_PLAN_WITH_VISUAL_FIX, encoding="utf-8")
        return PlanExecLoopResult(
            run_dir=config.run_dir,
            final_status=FinalStatus.COMPLETED,
            plan_iter_count=1,
    ...
    assert plan_path.read_text(encoding="utf-8") == _PLAN_WITH_VISUAL_FIX
