...
Status: **Implemented** · Version: **0.1** with v0.2 additions in code
Owners: ShopBackend
...
- **Cart store** — the per-server mutable cart state. By default this
  is in-memory for the lifetime of a server instance. When
  `cartStorePath` / `--cart-store` is set, cart state is rehydrated
  from and persisted to a JSON snapshot file after mutations.

---

## 3. Current Status

- `packages/shop_backend/` is implemented as a TypeScript ESM package
  with graphql-yoga, generated resolver types, a dataset loader,
  resolvers, HTTP wrapper, CLI, and vitest coverage.
- The server boots against a SandboxShop dataset directory and exposes
  `/graphql`, `/api/<version>/graphql.json`, `/images/<path>`, and
  `/health`.
- The implemented Storefront-API subset covers shop/menu/localization,
  products, variants, collections, pages, policies, blogs/articles,
  search, predictive search, product recommendations, product /
  collection / shop metafields, `@inContext` country/language
  validation, inventory-backed `availableForSale`, and cart lifecycle
  mutations.
- Optional dataset files now include `blogs.json`, `metafields.json`,
  and `inventory.json`; absent optional files default to empty data.
- Cart state is in-memory by default and optionally persisted with the
  `--cart-store <path>` CLI flag / `ServerOptions.cartStorePath`.
- `shop_arena.gen` emits the dataset files consumed by this server and
  uses `shop_backend` for hosting validation and generated-shop serving.
...
```
shop-backend ./outputs/shops/example/data 4000 [--cart-store ./cart-store.json]
```
...
- Cart persistence by default. Cart state lives in-process unless
  `cartStorePath` / `--cart-store` is configured.
- Subscriptions, webhooks, real-time updates.
- Direct inventory-management queries. `inventory.json` currently
  backs product / variant availability; it is not exposed as its own
  buyer-facing query surface.
...
**`homepage.json`** — site-level editorial imagery reused verbatim from
the source storefront. Defaults to `{ hero: null, banners: [] }` if
absent (pre-homepage datasets and shops whose hero capture did not run
still load).

```ts
interface Homepage {
  hero: ProductImage | null;        // most prominent above-the-fold image
  banners: ProductImage[];          // ordered promo banners below the hero
}
```

`ProductImage` is the same shape as `products[].images[]` /
`collections[].image` (§8.1.1). The `src` values are typically absolute
CDN URLs hotlinked from the original store (passed through unchanged per
§8.1.4); `alt` is `null` to keep source brand copy out of the twin.
Served by the root `homepage: Homepage` query, whose `hero`/`banners`
resolve to `Image` nodes.
