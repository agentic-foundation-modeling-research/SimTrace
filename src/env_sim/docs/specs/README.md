...
| [shop_arena/clickstream_replay.md](shop_arena/clickstream_replay.md) | `packages/shop_arena/src/shop_arena/gen/build/verifiers` + `packages/shop_arena/src/shop_arena/gen/final_eval` | `clickstream_replay` verifier: replays real user clickstream trajectories against the generated twin with Playwright and gates on strict reproduction of each session's actions (opt-in via `--clickstream`); plus an advisory cross-iteration rollup merged into `final_eval.json`. **Status:** Implemented. |
