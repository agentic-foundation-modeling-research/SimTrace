...
Status: **v0.1 shipping; v0.2 orchestration redesign pending** · Version: **0.2** (incorporates the v2 redesign of the orchestration layer)
Owners: ShopArena
...
**Capabilities shipped:** M0–M7 (see §8) landed against the legacy
design. Full e2e (single + multi-seed) generates a hostable
SandboxShop; data validation, build-harness loop with caller-owned
verifiers, manual sub-splitting, `visual_fix`, `visual_judge`, and
final-eval visual sweep all run.
...
| `ingest_homepage`            | seed capture (best-effort)           | `data/homepage.json` — site-level hero + promo banners hotlinked verbatim from the real store |
...
- **`ingest_homepage`** — emits the site-level `data/homepage.json`
  (hero + promo banners) directly from the seed's optional
  `artifact/prefetch/hero.json` capture. Registered in **both** catalog
  modes (synth and ingest) because the editorial hero/banner imagery is
  reused from the real store regardless of how the catalog was produced.
  Each captured image becomes a `ProductImage` whose `src` is the real
  absolute CDN URL **verbatim** (hotlinked; the backend passes absolute
  URLs through untouched) with `alt=None`. Best-effort: a missing or
  malformed capture yields an empty `{hero: null, banners: []}` document,
  never an abort. This step writes under `data/` outside `assemble_data`
  because the homepage payload is copied verbatim and needs no allowlist
  scrub of invented prose.
...
- **`assemble_data`** — combines stage outputs into the final
  ...
  files. **This is the only step that writes the catalog under
  `data/`** (`ingest_homepage` owns `data/homepage.json`).
...
- [ ] visual_fix          [priority: 1]  — REQUIRED final task; cross-task cleanup (§5.5.4)
```
...
| `quality_judge`       | LLM  | post `gen_homepage`, `gen_product`, `gen_cart_search`, `visual_fix` | Render the page; LLM judges quality against `capabilities.json` keys. **Quality-only**, not a cross-compare with seeds. |
| `visual_judge`        | LLM  | post `gen_homepage`, `gen_product`, `gen_cart_search`, `visual_fix` | Boots a transient dev server and drives the playwright skill against the task's routes; LLM judges the **rendered** storefront against the bucket-filtered `capabilities.json` slice. Per-task retry budget; page-bucket fan-out for `visual_fix`. See [`visual_verifier.md`](visual_verifier.md). |
| `cross_task_consistency` | LLM | post `visual_fix`             | Whole-app sweep: are all generated components mutually consistent (shared design tokens, shared types, no orphan imports, navigation matches collections)? |
...
#### 5.5.4 4.4 The `visual_fix` task (cross-ownership cleanup)
...
> You are the visual-fix pass. Every other gen task has
...
Verifiers gate `visual_fix` like any other task — `tsc`, `build`,
`quality_judge`, plus the visual-fix-only `cross_task_consistency`
LLM judge. A `FAIL` rewrites
`visual_fix`'s marker back to `[~]` per the standard verifier
contract; the next iteration retries with feedback. If the
visual-fix task budget exhausts without converging, the run
...
The intended `no_brand_leak` verifier (§5.5.3) would run the same
scanner over `hydrogen/app/**`, fail leaking iterations, and feed
`{{verifier_feedback}}` back to the agent. It is currently disabled
for the reasons captured below.
...
2. **Verifier-injected re-plans.** When `quality_judge` fails on
   ...
   `visual_fix` task (§5.5.4) and `harness redo <task_id>`
...
- **M8 (post-v0.1) — Image generation.** Pluggable replacement for
  the placeholder generator. Implemented as the `openai` image backend
  behind `--image-backend openai`.
- **M8a (post-v0.1) — Manual split.** Implemented
  [`manual_split.md`](manual_split.md): `split_manual_parts` writes
  area-scoped manuals and the build loop uses `visual_fix` instead of
  `consolidate`.
- **M8b (post-v0.1) — Visual verifier.** Implemented
  [`visual_verifier.md`](visual_verifier.md): `visual_judge`,
  configurable LLM judges, retry budget, routes/bucket helpers, and
  final-eval visual sweep.
